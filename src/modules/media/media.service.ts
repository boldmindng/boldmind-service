...
export class MediaService {
  ...
  async uploadMultiple(
    ...
    if (files.length > 10)
      throw new BadRequestException("Max 10 files per upload");
  ...
  // ─── GET SINGLE MEDIA RECORD (owner or admin) ───────────────────────────────

  async getMediaById(mediaId: string, userId: string, role?: string) {
    const media = await this.prisma.media.findFirst({ where: { id: mediaId } });
    if (!media) throw new NotFoundException("Media not found");
  ...
  // ─── SIGNED URL FOR A MEDIA RECORD (owner or admin, id-based) ───────────────
  ...
  async getSignedUrlForMedia(
    ...
    const media = await this.getMediaById(mediaId, userId, role); // 404/400 reuse
    const clampedExpiry = Math.min(Math.max(expiresIn, 60), 86400); // clamp 1min–24h
    const url = await this.getSignedReadUrl(media.key, clampedExpiry);
    return { url, key: media.key, expiresIn: clampedExpiry };
  }
}
